# Get relationship between time and probability of sharing (conservation)
...
T.all=c(1e5,1e6,1e7,1e8) # Time (may not be enough to reach stationary distribution, final dependent on initial state)

# Test run
out=rep(0,length(T.all))
Ne=1e4
alpha=1
nloci=5
trans=1
...
par=list(c(opt,10),1e-3)
mat=tm(nloci,u,par,Ne,alpha,trans,C,espsilon,gamma_0,gamma_1)
start=rep(0,nloci+1);start[length(start)]=1
for(t in 1:length(T.all)){
	distr=start%*%(mat %^% T.all[t])
	out[t]=1-distr[1]
}
...
out=matrix(0,nrow=nrow(comb.all),ncol=length(T.all))
for(i in 1:nrow(comb.all)){
	...
	for(t in 1:length(T.all)){
		distr=start%*%(mat %^% T.all[t])
		if(distr[1]>1){ # If the probability is >1, set it to 1
			distr[1]=1
		}
		out[i,t]=(1-distr[1])^2 # Probability that cis-genotypic value is >0 in two lineages
	}
}

out=data.frame(comb.all,out)
colnames(out)=c("Ne","alpha","l","Q","u01","u10","prob1","prob2","prob3","prob4")
out$alpha=log(out$alpha)
write.table(out,file="out_basic_cons.txt",sep="\t")


# Plot a subset
d<-read.table("out_basic_cons.txt",sep="\t",header=TRUE)
...
g<-ggplot(dsub,aes(x=log10(Ne),y=prob4,colour=l))
g=g+geom_point()+geom_line()+scale_color_brewer(palette="Paired")
g=g+theme_classic()
g=g+xlab(expression(paste("Lo",g[10],N[e])))+ylab("Probability of sharing")
...
ggsave("plot_basic_cons_1e8.pdf",plot=g,width=6,height=5)
#ggsave("plot_basic_cons_1e8_mb1.pdf",plot=g,width=6,height=5)
#ggsave("plot_basic_cons_1e8_mb2.pdf",plot=g,width=6,height=5)
g<-ggplot(dsub,aes(x=log10(Ne),y=prob3,colour=l))
g=g+geom_point()+geom_line()+scale_color_brewer(palette="Paired")
g=g+theme_classic()
g=g+xlab(expression(paste("Lo",g[10],N[e])))+ylab("Probability of sharing")
lt=expression(paste("Number of ",italic(cis),"- loci"))
...
g=g+ylim(c(0,1))
g=g+theme(axis.text=element_text(size=12),axis.title=element_text(size=15),legend.text=element_text(size=15),legend.title=element_text(size=15),legend.key.size=unit(0.7,'cm')) # Adjust font size of axis labels, axis text, and legend
ggsave("plot_basic_cons_1e7.pdf",plot=g,width=6,height=5)
#ggsave("plot_basic_cons_1e7_mb1.pdf",plot=g,width=6,height=5)
#ggsave("plot_basic_cons_1e7_mb2.pdf",plot=g,width=6,height=5)
